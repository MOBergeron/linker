...
from linker.parsers import CrackedHashesParser, SecretsDumpParser, ntHashRegex
...
def _evaluate_password_patterns(entries, dl_threshold):
    ...
    if avg_norm_dist < dl_threshold:
        return (
            True,
            "predictable password history (avg normalized Damerau-Levenshtein distance {:.2f}, threshold {:.2f})".format(
    ...
    for acc_id, acc_list in groups.items():
        ...
        for acc in acc_list:
            if acc.password is not None:
                label = (
                    "current"
                    if acc.entry_type == Account.ENTRY_CURRENT
                    else "history{}".format(acc.entry_index)
                )
                entries.append((label, acc.password))
        if len(entries) < 2:
            continue
        predictable, desc = _evaluate_password_patterns(
            entries, dl_threshold=dl_threshold
        )
        if predictable:
...
def correlation(accounts, crackedDict, **kwargs):
    ...
    for account in accounts:
        if (
            kwargs["showMatchingDomain"]
            and (
                not account.domain
                or not kwargs["showMatchingDomain"].match(account.domain)
            )
        ) and (
            kwargs["showMatchingDomainsFile"]
            and (
                not account.domain
                or not account.domain in kwargs["showMatchingDomainsFile"]
            )
        ):
            continue
...
def _highlight_match(account, kwargs):
    return (
        (kwargs.get("highlightUser") and kwargs["highlightUser"].match(account.name))
        or (
            kwargs.get("highlightUsersFile")
            and account.name in kwargs["highlightUsersFile"]
        )
        or (
            kwargs.get("showMatchingPassword")
            and account.password in kwargs["showMatchingPassword"]
        )
        or (
            kwargs.get("showMatchingNTHash")
            and kwargs["showMatchingNTHash"].match(account.ntHash)
        )
        or (
            kwargs.get("showMatchingNTHashesFile")
    ...
    **kwargs,
    ...
    if kwargs["highlightOnly"]:
        ...
        if kwargs["showDisabled"]:
            showOnlyDisabled = len(
                [a for a in disabledAcc if _highlight_match(a, kwargs)]
            )
        if kwargs["showUncracked"]:
            showOnlyUncrackedEnabled = len(
                [a for a in uncrackedEnAcc if _highlight_match(a, kwargs)]
            )
            if kwargs["showDisabled"]:
                showOnlyUncrackedDisabled = len(
                    [a for a in uncrackedDisAcc if _highlight_match(a, kwargs)]
                )

    if kwargs.get("includeHistory"):
        enabled_current = {
            a for a in enabledAcc if a.entry_type == Account.ENTRY_CURRENT
        }
        enabled_history = {
            a for a in enabledAcc if a.entry_type == Account.ENTRY_HISTORY
        }
        disabled_current = {
            a for a in disabledAcc if a.entry_type == Account.ENTRY_CURRENT
        }
        disabled_history = {
            a for a in disabledAcc if a.entry_type == Account.ENTRY_HISTORY
        }
        showOnlyEnabledCurrent = (
            len([a for a in enabled_current if _highlight_match(a, kwargs)])
            if kwargs["highlightOnly"]
            else 0
        )
        showOnlyEnabledHistory = (
            len([a for a in enabled_history if _highlight_match(a, kwargs)])
            if kwargs["highlightOnly"]
            else 0
        )
        showOnlyDisabledCurrent = (
            len([a for a in disabled_current if _highlight_match(a, kwargs)])
            if kwargs["highlightOnly"]
            else 0
        )
        showOnlyDisabledHistory = (
            len([a for a in disabled_history if _highlight_match(a, kwargs)])
            if kwargs["highlightOnly"]
            else 0
        )
        showEnabledAccounts(
            enabled_current,
            showOnlyEnabledCurrent,
            section_title="Enabled accounts",
            **kwargs,
        )
        showEnabledAccounts(
            enabled_history,
            showOnlyEnabledHistory,
            section_title="Enabled accounts history",
            **kwargs,
        )
        showDisabledAccounts(
            disabled_current,
            showOnlyDisabledCurrent,
            section_title="Disabled accounts",
            **kwargs,
        )
        showDisabledAccounts(
            disabled_history,
            showOnlyDisabledHistory,
            section_title="Disabled accounts history",
            **kwargs,
        )
    else:
    ...
    if kwargs.get("includeHistory") and kwargs["showUncracked"]:
        uncracked_en_current = {
            a for a in uncrackedEnAcc if a.entry_type == Account.ENTRY_CURRENT
        }
        uncracked_en_history = {
            a for a in uncrackedEnAcc if a.entry_type == Account.ENTRY_HISTORY
        }
        uncracked_dis_current = {
            a for a in uncrackedDisAcc if a.entry_type == Account.ENTRY_CURRENT
        }
        uncracked_dis_history = {
            a for a in uncrackedDisAcc if a.entry_type == Account.ENTRY_HISTORY
        }
        showOnlyUncrackedEnCurrent = (
            len([a for a in uncracked_en_current if _highlight_match(a, kwargs)])
            if kwargs["highlightOnly"]
            else 0
        )
        showOnlyUncrackedEnHistory = (
            len([a for a in uncracked_en_history if _highlight_match(a, kwargs)])
            if kwargs["highlightOnly"]
            else 0
        )
        showOnlyUncrackedDisCurrent = (
            len([a for a in uncracked_dis_current if _highlight_match(a, kwargs)])
            if kwargs["highlightOnly"]
            else 0
        )
        showOnlyUncrackedDisHistory = (
            len([a for a in uncracked_dis_history if _highlight_match(a, kwargs)])
            if kwargs["highlightOnly"]
            else 0
        )
        _showUncrackedSection(
            uncracked_en_current,
            showOnlyUncrackedEnCurrent,
            "Uncracked enabled accounts",
            **kwargs,
        )
        _showUncrackedSection(
            uncracked_en_history,
            showOnlyUncrackedEnHistory,
            "Uncracked enabled accounts history",
            **kwargs,
        )
        _showUncrackedSection(
            uncracked_dis_current,
            showOnlyUncrackedDisCurrent,
            "Uncracked disabled accounts",
            **kwargs,
        )
        _showUncrackedSection(
            uncracked_dis_history,
            showOnlyUncrackedDisHistory,
            "Uncracked disabled accounts history",
            **kwargs,
        )
    else:
        showUncrackedAccounts(
            uncrackedEnAcc,
            showOnlyUncrackedEnabled,
            uncrackedDisAcc,
            showOnlyUncrackedDisabled,
            **kwargs,
        )
    showMatchingPassword(enabledAcc, disabledAcc, **kwargs)
    showMatchingNTHash(
        enabledAcc, disabledAcc, uncrackedEnAcc, uncrackedDisAcc, **kwargs
    )
    showMatchingNTHashesFile(
        enabledAcc, disabledAcc, uncrackedEnAcc, uncrackedDisAcc, **kwargs
    )
    showPasswordReuse(
        enabledAcc,
...
def showMatchingNTHash(
    enabledAcc, disabledAcc, uncrackedEnAcc, uncrackedDisAcc, **kwargs
):
    if kwargs["showMatchingNTHash"]:
        accounts = enabledAcc.union(disabledAcc, uncrackedEnAcc, uncrackedDisAcc)

        result = findAccountsWithNTHash(
            accounts, kwargs["showMatchingNTHash"], **kwargs
        )
        if result:
            print(
                "Accounts matching NTHash {}".format(
                    kwargs["showMatchingNTHash"].pattern
                )
            )
            for account in result:
            ...
            print(
                "No account found matching NTHash {}".format(
                    kwargs["showMatchingNTHash"]
                )
            )
...
def showPredictablePatterns(allAccounts, crackedDict, **kwargs):
    ...
    for acc_id, base_name, domain, pattern_desc, entries in sorting(
        results, key=lambda x: (x[2], x[1]), **kwargs
    ):
        display = "{}\\{}".format(domain, base_name) if domain else base_name
        print(
            "{tab}ID {id} ({display}): {desc}".format(
                tab=tab, id=acc_id, display=display, desc=pattern_desc
            )
        )
...
def showPasswordReuse(
    enabledAcc, disabledAcc, uncrackedEnAcc, uncrackedDisAcc, passwordCount, **kwargs
):
    if kwargs["showPasswordReuse"] and kwargs["showPasswordReuse"] > 1:
        ...
        for password, count in dict(
            sorting(
                passwordCount.items(),
                key=lambda x: x[1][True] + x[1][False],
                reverse=True,
                **kwargs,
            )
        ).items():
            if (
                (kwargs["showDisabled"] and count[True] + count[False] > 1)
                or (not kwargs["showDisabled"] and count[True] > 1)
            ) and (
                kwargs["showPasswordReuse"] == -1
                or count[True] + count[False] <= kwargs["showPasswordReuse"]
            ):
                ...
                if result:
                    if ntHashRegex.match(password):
                        ...
                        print(
                            "Possible password reuse for password {}".format(password)
                        )
                    for account in result:
                        r = formatResult(
                            account, False, showPasswordReusePassword=password, **kwargs
                        )
                        if r:
                            print(r)
                else:
                    if kwargs["showDisabled"]:
                        ...
                        print(
                            "No enabled account found with password {}".format(password)
                        )
...
def _print_stats_block(
    enabled, disabled, uncracked_en, uncracked_dis, spacing, block_title=None, **kwargs
):
    ...
    pCrackedEnabled = (
        decimal.Decimal((nbEnabled / totalEnabled) * 100) if totalEnabled != 0 else 0
    )
    pCrackedDisabled = (
        decimal.Decimal((nbDisabled / totalDisabled) * 100) if totalDisabled != 0 else 0
    )
    ...
    print(
        "{tab}Number of cracked enabled (en):{padding}{percent}".format(
            tab=tab * 2,
            padding=" " * (spacing - 35 - len(tab) - len(str(nbEnabled))),
            percent=nbEnabled,
        )
    )
    print(
        "{tab}Number of uncracked enabled (ue):{padding}{percent}".format(
            tab=tab * 2,
            padding=" " * (spacing - 37 - len(tab) - len(str(nbUncrackedEnabled))),
            percent=nbUncrackedEnabled,
        )
    )
    print(
        "{tab}Total of enabled (te=en+ue):{padding}{percent}".format(
            tab=tab * 2,
            padding=" " * (spacing - 32 - len(tab) - len(str(totalEnabled))),
            percent=totalEnabled,
        )
    )
    print(
        "{tab}Percentage of cracked (en/te):{padding}{percent}%".format(
            tab=tab * 2,
            padding=" " * (spacing - 34 - len(tab) - len(str(pCrackedEnabled)) - 1),
            percent=pCrackedEnabled,
        )
    )
    print("")
    print("{tab}Disabled Accounts".format(tab=tab))
    print(
        "{tab}Number of cracked disabled (dis):{padding}{percent}".format(
            tab=tab * 2,
            padding=" " * (spacing - 37 - len(tab) - len(str(nbDisabled))),
            percent=nbDisabled,
        )
    )
    print(
        "{tab}Number of uncracked disabled (ud):{padding}{percent}".format(
            tab=tab * 2,
            padding=" " * (spacing - 38 - len(tab) - len(str(nbUncrackedDisabled))),
            percent=nbUncrackedDisabled,
        )
    )
    print(
        "{tab}Total of disabled (td=dis+ud):{padding}{percent}".format(
            tab=tab * 2,
            padding=" " * (spacing - 34 - len(tab) - len(str(totalDisabled))),
            percent=totalDisabled,
        )
    )
    print(
        "{tab}Percentage of cracked (dis/td):{padding}{percent}%".format(
            tab=tab * 2,
            padding=" " * (spacing - 35 - len(tab) - len(str(pCrackedDisabled)) - 1),
            percent=pCrackedDisabled,
        )
    )
    print("")
    print("{tab}Every Account".format(tab=tab))
    print(
        "{tab}Number of cracked (c=en+dis):{padding}{percent}".format(
            tab=tab * 2,
            padding=" " * (spacing - 33 - len(tab) - len(str(nbCracked))),
            percent=nbCracked,
        )
    )
    print(
        "{tab}Number of uncracked (uc=ue+ud):{padding}{percent}".format(
            tab=tab * 2,
            padding=" " * (spacing - 35 - len(tab) - len(str(nbUncracked))),
            percent=nbUncracked,
        )
    )
    print(
        "{tab}Total (t=c+uc):{padding}{percent}".format(
            tab=tab * 2,
            padding=" " * (spacing - 19 - len(tab) - len(str(total))),
            percent=total,
        )
    )
    print(
        "{tab}Percentage of cracked (en/t):{padding}{percent}%".format(
            tab=tab * 2,
            padding=" " * (spacing - 33 - len(tab) - len(str(pCracked)) - 1),
            percent=pCracked,
        )
    )
    print(
        "{tab}Percentage of cracked (dis/t):{padding}{percent}%".format(
            tab=tab * 2,
            padding=" " * (spacing - 34 - len(tab) - len(str(pDisCracked)) - 1),
            percent=pDisCracked,
        )
    )
    print(
        "{tab}Percentage of cracked ((en+dis)/t):{padding}{percent}%".format(
            tab=tab * 2,
            padding=" " * (spacing - 39 - len(tab) - len(str(pTotalCracked)) - 1),
            percent=pTotalCracked,
        )
    )
...
def statistics(
    ...
    total_current = (
        len(enabled_current)
        + len(disabled_current)
        + len(uncracked_en_current)
        + len(uncracked_dis_current)
    )
    if total_current > 0:
        print("Statistics:")
        _print_stats_block(
        ...
        for k, v in dict(
            ...
            if kwargs["showDisabled"]:
                ...
                if v[True] > 1:
                    ...
                    continue
            print(
                "{tab}{isHash}{password}{padding}{value}".format(
                    tab=tab * 2,
                    isHash="[hash] " if ntHashRegex.match(k) else "",
                    password=k,
                    padding=" "
                    * (
                        spacing
                        ...
                        - len(k)
                        - len(value)
                        - (len("[hash] ") if ntHashRegex.match(k) else 0)
                    ),
                    value=value,
                )
            )
    ...
    total_history = (
        len(enabled_history)
        + len(disabled_history)
        + len(uncracked_en_history)
        + len(uncracked_dis_history)
    )
    if total_history > 0:
...
def formatResult(account, showPassword=True, **kwargs):
    ...
    if showPassword and account.password:
        if (
            kwargs["highlightUser"] and kwargs["highlightUser"].match(account.name)
        ) or (
            kwargs["highlightUsersFile"]
            and account.name in kwargs["highlightUsersFile"]
        ):
            if account.status:
                result = "\033[92m{tab}{accInfo}{padding}{password}\033[00m".format(
                    tab=tab,
                    accInfo=p,
                    padding=" " * (100 - len(tab) - len(p) - len(account.password)),
                    password=account.password,
                )
            else:
                result = "\033[91m{tab}{accInfo}{padding}{password}\033[00m".format(
                    tab=tab,
                    accInfo=p,
                    padding=" " * (100 - len(tab) - len(p) - len(account.password)),
                    password=account.password,
            ...
            kwargs["showMatchingPassword"]
            and account.password in kwargs["showMatchingPassword"]
        ):
            if account.status:
                result = "\033[93m{tab}{accInfo}{padding}{password}\033[00m".format(
                    tab=tab,
                    accInfo=p,
                    padding=" " * (100 - len(tab) - len(p) - len(account.password)),
                    password=account.password,
            ...
            result = "{tab}{accInfo}{padding}{password}".format(
                tab=tab,
                accInfo=p,
                padding=" " * (100 - len(tab) - len(p) - len(account.password)),
                password=account.password,
        ...
        if (
            ...
            kwargs["showPasswordReuse"]
            and "showPasswordReusePassword" in kwargs
            and account.password == kwargs["showPasswordReusePassword"]
        ):
            if account.status:
                result = "\033[92m{tab}{accInfo}\033[00m".format(tab=tab, accInfo=p)
            else:
...
def main():
    ...
    parser.add_argument(
        metavar="dump_file",
        dest="dump",
        help="Dump file.\nLine format : [DOMAIN\\]USERNAME:USERID:LM:NT::: (status=(Dis|En)abled)",
        type=str,
    )
    parser.add_argument(
        metavar="cracked_file", dest="cracked", help="Cracked hash file.", type=str
    )
    parser.add_argument(
        "-a", "--all", dest="all", help="Show everything.", action="store_true"
    )
    parser.add_argument(
        "-v", "--verbose", dest="verbose", help="Verbose mode.", action="store_true"
    )
    parser.add_argument(
        "-i",
        "--disabled",
        dest="showDisabled",
        help="Show disabled in the result.\nDefault status is 'enabled' if not present in hashes file.",
        action="store_true",
    )
    parser.add_argument(
        "-u",
        "--uncracked",
        dest="showUncracked",
        help="Show uncracked accounts in the result.",
        action="store_true",
    )
    parser.add_argument(
        "-n",
        "--nthash",
        dest="showNTHash",
        help="Show NTHash in the result.",
        action="store_true",
    )
    parser.add_argument(
        "-d",
        "--showdomain",
        dest="showDomain",
        help="Show domain in the result.",
        action="store_true",
    )
    parser.add_argument(
        "-s",
        "--stats",
        dest="showStats",
        help="Show statistics in the result.",
        action="store_true",
    )
    parser.add_argument(
        "-r",
        "--password-reuse",
        dest="showPasswordReuse",
        help="Show accounts with the same password in result. Use '-r all' to show them all. (If --all is used: default shows password reused twice).",
        type=str,
        default=None,
    )
    parser.add_argument(
        "-D",
        "--domain",
        dest="showMatchingDomain",
        help="Show only matching domain  (regex).",
        type=str.upper,
        default=None,
    )
    parser.add_argument(
        "--domains-file",
        dest="showMatchingDomainsFile",
        help="Show only matching domains by providing a file splitted by newlines.",
        type=str,
        default=None,
    )
    parser.add_argument(
        "-P",
        "--password",
        dest="showMatchingPassword",
        help="Show matching password in the result.",
        type=str,
        default=None,
    )
    parser.add_argument(
        "--passwords-file",
        dest="showMatchingPasswordsFile",
        help="Show matching password in the result by providing a file splitted by newlines.",
        type=str,
        default=None,
    )
    parser.add_argument(
        "-N",
        "--hash",
        dest="showMatchingNTHash",
        help="Show matching hash in the result (regex).",
        type=str.lower,
        default=None,
    )
    parser.add_argument(
        "--hashes-file",
        dest="showMatchingNTHashesFile",
        help="Show matching hash in the result by providing a file splitted by newlines.",
        type=str.lower,
        default=None,
    )
    parser.add_argument(
        ...
        "--user",
        dest="highlightUser",
        help="Highlight matching user in the result (regex).",
        type=str.lower,
        default=None,
    )
    parser.add_argument(
        "--users-file",
        dest="highlightUsersFile",
        help="Highlight matching users in the result by providing a file splitted by newlines.",
        type=str,
        default=None,
    )
    parser.add_argument(
        ...
        "--highlight-only",
        dest="highlightOnly",
        help="Show only highlists results (requires showMatchingDomain, showMatchingPassword, showMatchingNTHash or highlightUser).",
        action="store_true",
    )
    parser.add_argument(
        "-pp",
        "--predictable-patterns",
        dest="showPredictablePatterns",
        help="Group accounts by ID, evaluate password history similarity, report users with predictable patterns.",
        action="store_true",
    )
    parser.add_argument(
        "-dl",
        ...
        type=float,
        default=0.25,
        help="Max avg normalized Damerau-Levenshtein distance below which password history is flagged as predictable; lower = stricter (default: %(default)s).",
    )
    parser.add_argument(
        "-hi",
        "--history",
        dest="includeHistory",
        help="Include historical password entries (entry_history0, entry_history1, ...) in stats: enabled/disabled/uncracked counts, password reuse, statistics. Default: only current entry per account. Predictable-patterns check always uses history.",
        action="store_true",
    )
    parser.add_argument(
        "-p",
        "--performance",
        dest="performance",
        help="Need more performance? This will NOT sort the results.",
        action="store_true",
    )
    parser.add_argument(
        "-t",
        "--time",
        dest="time",
        help="Print the elasped time to run the script.",
        action="store_true",
    )
    ...
    if args.showMatchingDomain:
        if not args.showMatchingDomain.startswith("^"):
            args.showMatchingDomain = "^" + args.showMatchingDomain
        if not args.showMatchingDomain.endswith("$"):
            args.showMatchingDomain += "$"
    ...
    if args.showMatchingDomainsFile:
        if os.path.isfile(args.showMatchingDomainsFile):
            ...
            raise FileNotFoundError(
                "{} was not found.".format(args.showMatchingDomainsFile)
            )
    ...
    if args.showMatchingPasswordsFile:
        if os.path.isfile(args.showMatchingPasswordsFile):
            ...
            raise FileNotFoundError(
                "{} was not found.".format(args.showMatchingPasswordsFile)
            )

    if args.showMatchingNTHash:
        if not args.showMatchingNTHash.startswith("^"):
            args.showMatchingNTHash = "^" + args.showMatchingNTHash
        if not args.showMatchingNTHash.endswith("$"):
            args.showMatchingNTHash += "$"
    ...
    if args.showMatchingNTHashesFile:
        if os.path.isfile(args.showMatchingNTHashesFile):
            ...
            raise FileNotFoundError(
                "{} was not found.".format(args.showMatchingNTHashesFile)
            )

    if args.highlightUser:
        if not args.highlightUser.startswith("^"):
            args.highlightUser = "^" + args.highlightUser
        if not args.highlightUser.endswith("$"):
            args.highlightUser += "$"
    ...
    if args.highlightOnly:
        if (
            args.showMatchingDomain
            or args.showMatchingPassword
            or args.showMatchingNTHash
            or args.highlightUser
            or args.showMatchingDomainsFile
            or args.showMatchingPasswordsFile
            or args.showMatchingNTHashesFile
            or args.highlightUsersFile
        ):
    ...
    if args.showPasswordReuse:
        if args.showPasswordReuse == "all":
            ...
            raise ValueError(
                "'{}' is not an integer or 'all'.".format(args.showPasswordReuse)
            )
    else:
        if args.all:
            args.showPasswordReuse = INT_MAXSIZE
    ...
    if args.all:
        ...
        args.showPasswordReuse = (
            INT_MAXSIZE if args.showPasswordReuse is None else args.showPasswordReuse
        )
    ...
    if args.cracked and (
        not os.path.exists(args.cracked) or not os.path.isfile(args.cracked)
    ):
    ...
    accounts_for_stats = (
        ...
        else {a for a in allAccounts if a.entry_type == Account.ENTRY_CURRENT}
    )
    (enabledAcc, disabledAcc, uncrackedEnAcc, uncrackedDisAcc, passwordCount) = (
        correlation(accounts_for_stats, crackedDict, **vars(args))
    )
    showResults(
        allAccounts,
        crackedDict,
